...
describe('createWishlist', () => {
  it('returns null when not enabled', () => {
    expect(createWishlist(false, false, 'h1', undefined, [])).toBeNull();
    expect(createWishlist(undefined, false, 'h1', undefined, [])).toBeNull();
  });

  it('renders wishlist button', () => {
    const result = createWishlist(true, false, 'h1', undefined, []);
  ...
  it('sets aria-pressed to true when wishlisted', () => {
    const result = createWishlist(true, true, 'h1', undefined, []);
  ...
  it('calls onToggle callback with correct values', () => {
    const onToggle = vi.fn();
    const result = createWishlist(true, false, 'h1', onToggle, []);

    result!.element.click();
    expect(onToggle).toHaveBeenCalledWith(true, 'h1');

    result!.element.click();
    expect(onToggle).toHaveBeenCalledWith(false, 'h1');
  });

  it('pushes cleanup to array', () => {
    const cleanups: (() => void)[] = [];
    createWishlist(true, false, 'h1', undefined, cleanups);
