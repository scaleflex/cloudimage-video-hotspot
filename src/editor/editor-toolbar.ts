...
const ICONS = {
  ...
  video: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect width="20" height="15" x="2" y="4" rx="2"/><path d="m10 9 5 3-5 3z"/></svg>',
...
export class EditorToolbar {
  ...
  private build(): void {
    ...
    this.trackBtn = this.createBtn(ICONS.crosshair, 'Track', () => this.editor.setMode('track'));
    modeGroup.appendChild(this.selectBtn);
    modeGroup.appendChild(this.addBtn);
    modeGroup.appendChild(this.trackBtn);
    ...
    const loadBtn = this.createBtn('', 'Load', () => this.loadVideoUrl(urlInput));
    urlInput.addEventListener('keydown', (e) => {
      if (e.key === 'Enter') {
        e.preventDefault();
        this.loadVideoUrl(urlInput);
      }
    });
  ...
  updateState(): void {
    ...
    this.trackBtn.classList.toggle('ci-editor-btn--active', mode === 'track');
    this.selectBtn.setAttribute('aria-pressed', String(mode === 'select'));
    this.addBtn.setAttribute('aria-pressed', String(mode === 'add'));
    this.trackBtn.setAttribute('aria-pressed', String(mode === 'track'));
  ...
  private showExportJSON(): void {
    ...
    this.editor.showToast('Exported video-hotspots.json');
  }
  ...
  private showImportModal(): void {
    ...
    title.id = titleId;
    title.textContent = 'Import Video Hotspots JSON';

    const textarea = createElement('textarea');
    textarea.placeholder = '[\n  { "id": "h1", "x": "50%", "y": "50%", "startTime": 5, "endTime": 20, "label": "My Hotspot" }\n]';
  ...
  private loadVideoUrl(input: HTMLInputElement): void {
    const src = input.value.trim();
    if (!src) return;
    this.editor.setSrc(src);
    this.editor.showToast('Video updated');
  }
